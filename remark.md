...
- [x] **情緒粒子系統 (Emote Particle System)**：
    -   移除優先級仲裁機制 (Arbitration)，改為並發顯示。
    -   所有情緒符號 (Emotes) 視為獨立粒子，生成後自動上飄並淡出 (Float & Fade)。
    -   支援多個表情同時顯示，增強視覺豐富度與即時回饋。
...
*   **撞牆 (Wall Collision)**：
    *   若冒險者被彈飛後撞到牆壁，將受到額外傷害並暈眩一回合。
    *   這鼓勵玩家設計將冒險者彈向牆壁的陷阱組合。



### G. 狀態效果 (Status Effects)
- **油膩 (Oiled)**:
    - **效果**: 移動時累積 "Grease" 計數器。
    - **觸發**: 當在油膩狀態下移動 **3步** 後，觸發 **"滑倒 (Slip)"** 事件。
    - **滑倒後果**: 冒險者播放滑倒動畫 (旋轉/搖晃)，**暈眩 (Stunned)** 2秒，並移除油膩狀態。
- **揮發引燃 (Volatile Ignition)**:
    - **觸發**: 當 **Oiled** 冒險者試圖踏上 **Fire** 屬性陷阱（如噴火口、營火）時。
    - **效果**: 立即受到 **30 火焰傷害**（因油氣引爆），移除油膩狀態，並**中斷本次移動**（被擊退）。此機制優先於「陷阱恐慌」。

### F. 陷阱耐久度 (Trap Durability)
- **機制定義**：陷阱擁有「剩餘觸發次數」。
- **耗盡移除**：當次數歸零時，陷阱會自動從地圖上移除（銷毀）。
- **設定值**：
    - **無限陷阱**：如尖刺、元素地塊 (油/火/水/雷)。耐久度設為 -1，永不損壞。
    - **消耗型陷阱**：如彈簧 (Spring)。耐久度設為 1，觸發一次後即損壞並消失。
- **策略影響**：玩家不能再依賴單一彈簧無限彈飛敵人，需要計算「耗材」成本與敵人數量的比例。
...
*   **陷阱焦慮 (Trap Anxiety)**：
    *   **檢測**: 移動前會檢查前方是否有 **「恐怖陷阱」(Scary Traps)**（如尖刺、火）。
    *   **恐慌 (Panic)**: 首次發現恐怖陷阱時，且該陷阱具有 `isScary: true` 屬性，冒險者會觸發 **「試探與驚恐」(Peek & Panic)** 行為：
        1.  **試探 (Probe)**: 身體前傾踏入格子 40%。
